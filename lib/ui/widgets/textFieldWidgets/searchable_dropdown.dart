...
Widget prepareWidget(dynamic object,
  ...
  if (object is String) {
    if (stringToWidgetFunction == null) {
      ...
      return stringToWidgetFunction(object) as Widget;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    selectedItems?.forEach((item) {
      list.add(widget.selectedValueWidgetFn != null
          ? widget.selectedValueWidgetFn(widget.items[item].value) as Widget
          : items[item]);
    });
      ...
      children: <Widget>[
        ...
        if (valid)
          ...
          validatorOutput is String
              ...
              : validatorOutput as Widget,
        if (displayMenu.value) menuWidget else const SizedBox.shrink(),
      ],
    );
  }
...
class _DropdownDialogState<T> extends State<DropdownDialog> {
  ...
  void _updateShownIndexes(String keyword) {
    shownIndexes = searchFn(keyword, widget.items) as List<int>;
  }
    ...
    if (widget.searchFn != null) {
      ...
      if (widget.isCaseSensitiveSearch) {
        matchFn = (item, keyword) {
          return item.value.toString().contains(keyword as Pattern);
        };
      } else {
        matchFn = (item, keyword) {
          return item.value
              .toString()
              .toLowerCase()
              .contains(keyword.toLowerCase() as Pattern);
        };
      }
      searchFn = (keyword, items) {
        ...
        widget.items.forEach((item) {
          if ((matchFn(item, keyword) as bool) ||
              (keyword?.isEmpty ?? true) as bool) {
            shownIndexes.add(i);
          }
          i++;
        });
  ...
  Widget titleBar() {
    ...
    final Widget validatorOutputWidget = valid
        ? const SizedBox.shrink()
        : validatorOutput is String
            ? Text(
                validatorOutput,
              ...
              child: widget.multipleSelection
                  ? widget.displayItem == null
                      ...
                      : widget.displayItem(
                          item,
                          widget.selectedItems
                              .contains(shownIndexes[index])) as Widget
                  : widget.displayItem == null
                      ? item
